...
namespace UnitTestProject1
{
    ...
    public class UnitTestCopier
    {
        ...
        public void Copier_Scan_FormatTypeDocument()
        {
            ...
            using (var consoleOutput = new ConsoleRedirectionToStringWriter())
            {
                copier.Scan(out _, formatType: IDocument.FormatType.JPG);
                Assert.IsTrue(consoleOutput.GetOutput().Contains("Scan"));
                Assert.IsTrue(consoleOutput.GetOutput().Contains(".jpg"));

                copier.Scan(out _, formatType: IDocument.FormatType.TXT);
                Assert.IsTrue(consoleOutput.GetOutput().Contains("Scan"));
                Assert.IsTrue(consoleOutput.GetOutput().Contains(".txt"));

                copier.Scan(out _, formatType: IDocument.FormatType.PDF);
                Assert.IsTrue(consoleOutput.GetOutput().Contains("Scan"));
